...
export class DeepgramProvider implements IsttProvider {
  ...
  async transcribe(
    ...
    const apiKey =
      ...
      this.configService.get<string>('DEEPGRAM_API');
    if (!apiKey) {
      this.logger.error(
        'Deepgram API Key is not configured (DEEPGRAM_API_KEY is missing).',
      );
      throw new InternalServerErrorException(
        'STT service configuration error: Missing credentials.',
      );
    }

    if (!audioBuffer || audioBuffer.length === 0) {
      throw new BadRequestException(
        'Invalid audio input: audio buffer is empty.',
      );
    }
    ...
    const smartFormat = this.configService.get<string>(
    ...
    this.logger.log(
      `Initiating transcription request with model=${model}, language=${language}`,
    );
    ...
    try {
      ...
      const responseLanguage =
        response.data?.results?.channels?.[0]?.detected_language || language;
      ...
      this.logger.error(
        `Deepgram transcription failed: Status=${status}, Message=${errorMsg}`,
      );

      if (status === 401 || status === 403) {
        throw new InternalServerErrorException(
          'Authentication failure with STT provider.',
        );
      } else if (status === 400) {
        throw new BadRequestException(
          `Bad request to STT provider: ${errorMsg}`,
        );
      }

      throw new InternalServerErrorException(
        `STT transcription failed: ${errorMsg}`,
      );
    }
  }
}
